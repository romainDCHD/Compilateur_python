...
static void list_basics( void ) {
  test_suite( "Basic test of list module, list of lexems (list_add_first, list_print, list_del_first, list_add_last)" );
  ...
  test_oracle( list_print_first( l, lexem_print ), "[1:0:str] ficelle",
  	     "Can print only the first element of the lista list w/ 2 elements");
  ...
  // ajout en à la fin d'un deuxieme element
  l = list_add_first( l, lexem_new( "int", "43", 2, 9 ) );
  test_oracle( list_print( l, lexem_print ), "( [2:9:int] 43 [1:8:int] 42 )",
        "List is OK after list_add_last()");
