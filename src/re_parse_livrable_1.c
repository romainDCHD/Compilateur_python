...
queue_t fillchargroup_2 (queue_t queue,char_group *chargroup){  //pour remplir la queue correctement
...
//----------------------------------------------------
    ...
    else if ( *data_in == '[' )
    {
      ...
      exit = read_crochet_2( data_in , chargroup );  //fill chargroup->group
      if (exit == 0)
      {
        return 0;
      }
...
//----------------------------------------------------
  //case '^'
    else if (*data_in == '^')
    {
      ...
      {
        data_in ++;
        exit = read_crochet_2( data_in , chargroup );
        if (exit == 0)
        {
          return 0;
        }
...
//----------------------------------------------------
    ...
    else
    {
      ...
      //si au bout et signe
      if ( (*data_in == '?' || *data_in == '+' || *data_in == '*') && *ptr == '\0' && queue_empty(*queue) == 0 )
      {
       data_in ++;
      }
